...
- 歌词使用霞鹜文楷字体（CDN），歌名使用 CDN 明体（Noto Serif TC/SC）+ 系统明体 fallback
- 支持换行显示，简繁双语
...
- **外部字体**：霞鹜文楷（歌词）、Noto Serif TC/SC（中文明体）、Space Grotesk（品牌英文）
...
- `npm run dev` — 启动开发服务器并监听 `0.0.0.0`（端口 5353，便于局域网设备访问）
- `npm run build` — 构建生产版本
- `npm run preview` — 预览生产构建并监听 `0.0.0.0`

## Deployment Status

构建成功，可部署。待办：
- [ ] iOS Safari 背景露顶/露底问题彻底修复（地址栏显隐、回弹场景）
- [ ] Footer 页脚（可选）
- [ ] SEO meta tags（可选）
- [ ] 移动端细节测试（含 hero / stickers / 背景在多机型回归）
